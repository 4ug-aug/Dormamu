...
interface AsanaSyncDialogProps {
  open: boolean;
...
export default function AsanaSyncDialog({
  open,
  onOpenChange,
}: AsanaSyncDialogProps) {
  const [entries, setEntries] = useState<AsanaSyncableEntry[]>([]);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (key) {
        const result = await invoke<AsanaSyncableEntry[]>("get_asana_syncable_entries");
        setEntries(result);
      }
    } catch (err) {
  ...
  const handleSync = async () => {
    ...
    setIsSyncing(true);
    try {
      const count = await invoke<number>("sync_entries_to_asana", {
        apiKey,
        entryIds: Array.from(selectedIds),
      });
      toast.success(`Synced ${count} time entry(ies) to Asana`);
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[550px] sm:max-h-[600px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Export time entries to Asana for tasks imported from Asana.
          </DialogDescription>
        </DialogHeader>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-6">
            <p className="text-muted-foreground mb-4">
              Please set up your Asana PAT first using "Import from Asana".
            </p>
          ...
          <div className="text-center py-6">
            <p className="text-muted-foreground mb-2">No time entries to sync.</p>
            <p className="text-xs text-muted-foreground mb-4">
              Only entries from tasks imported from Asana can be synced.
            </p>
            <Button variant="outline" onClick={handleClose}>
              Close
            </Button>
